...
export default function RunningAnalysisPanel() {
  const [timeRange, setTimeRange] = useState<TimeRange>({
    type: "quick",
    quickRange: "24h",
    absoluteFrom: "now-24h",
    absoluteTo: "now",
  });
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-grow">
        <iframe
          key={iframeKey}
          src={`http://localhost:3000/d-solo/1589c2c2-78af-482b-9e45-51f0e7595393/digitaltwin-dash-fv?orgId=1&from=${range.from}&to=${range.to}&timezone=browser&var-Filters=${filters.selectedFilter}&theme=light&panelId=2&__feature.dashboardSceneSolo=true`}
          className="w-full h-full border-0"
          title="Running Analysis Panel"
        />
      </div>
    </div>
  );
