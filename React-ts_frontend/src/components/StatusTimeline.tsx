...
const getTimeRange = (timeRange: TimeRange): TimeRangeResult => {
  if (timeRange.type === "quick") {
    ...
    return {
      from: parseTime(timeRange.absoluteFrom).getTime(),
      to: parseTime(timeRange.absoluteTo).getTime(),
    };
  }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-grow">
        <iframe
          key={iframeKey}
          src={`http://localhost:3000/d-solo/76a7e6c9-d7c5-4f28-ad45-caccb7d2499c/dashbord-dt?orgId=1&from=${range.from}&to=${range.to}&timezone=utc&var-sensor_id=dps_light1&var-controller_ref=DPS001&var-Filters=&theme=light&panelId=6&__feature.dashboardSceneSolo=true`}
