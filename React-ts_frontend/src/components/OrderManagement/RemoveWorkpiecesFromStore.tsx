...
import { X, Search, Trash, RefreshCw } from "lucide-react";
...
interface RemoveWorkpiecesResponse {
  id: string;
  status: string;
  requestedBy: string;
  createdAt: string;
...
export default function RemoveWorkpiecesFromStore() {
  ...
  const [removeResponse, setRemoveResponse] = useState<RemoveWorkpiecesResponse | null>(null);
  ...
  const fetchWorkpieces = async () => {
    try {
      ...
      const storedWPs = data.filter((wp) => wp.state?.includes("STORED"));
      setWorkpieces(storedWPs);
      setFilteredWorkpieces(storedWPs);
    } catch (err) {
      console.error("Failed to fetch workpieces:", err);
    }
  };
  ...
  useEffect(() => {
    const filtered = workpieces.filter(
      (wp) =>
        ...
        (wp.addedBy && wp.addedBy.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  ...
  const handleRemoveWorkpieces = async () => {
    if (!selectedWorkpieces.length) return;
    try {
      setRemoving(true);
      const res = await api.post<RemoveWorkpiecesResponse>("/api/Order/RemoveWorkpieces", {
        factoryId: "FACTORY001",
        requestedBy: "mahdi",
        workpieceIds: selectedWorkpieces,
      });
      setRemoveResponse(res.data);
      setSelectedWorkpieces([]);
      fetchWorkpieces();
      setShowModal(false);
    } catch (err) {
      console.error("Failed to remove workpieces:", err);
    } finally {
      setRemoving(false);
    }
  };
  ...
  return (
    <>
<OpenModalButton
  label="Remove Stored Workpieces"
  icon={<Trash className="w-5 h-5" />}
  className="w-full bg-white text-red-600 border-2 border-red-600 hover:bg-gradient-to-r hover:from-red-600 hover:to-red-700 hover:text-white py-3  font-medium transition-all duration-200"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg max-h-[80vh] flex flex-col">
            ...
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-800">Remove Workpieces</h3>
            ...
            <div className="flex gap-2 mb-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search by ID, Type, Added By..."
                  value={searchTerm}
            ...
            <div className="overflow-y-auto flex-1 border rounded-md p-2 bg-gray-50">
              {filteredWorkpieces.length === 0 ? (
                <div className="text-center text-gray-500 py-8">No STORED workpieces found.</div>
              ) : (
                  ...
                  {filteredWorkpieces.map((wp) => (
                    <li
                      ...
                      className={`flex justify-between items-center p-2 border rounded cursor-pointer transition ${
                        selectedWorkpieces.includes(wp.id) ? "bg-red-100" : "bg-white"
                      }`}
                    >
                      <div>
            ...
            <div className="flex justify-between items-center mt-4 pt-3 border-t">
              ...
              <button
                onClick={handleRemoveWorkpieces}
                disabled={removing || !selectedWorkpieces.length}
            ...
            {removeResponse && (
              <div className="p-2 mt-2 bg-green-100 text-green-800 rounded text-sm">
                Order created! ID: {removeResponse.id}, Status: {removeResponse.status}
              </div>
            )}
          </div>
        </div>
