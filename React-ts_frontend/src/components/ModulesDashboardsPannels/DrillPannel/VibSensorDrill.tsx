...
const quickRanges: QuickRange[] = [
  ...
  { label: "Last 2 days", value: "2d", from: () => new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), to: () => new Date() },
...
export default function VibSensorDrill() {
  const [timeRange, setTimeRange] = useState<TimeRange>({
    type: "quick",
    quickRange: "15m", // default last 15 minutes
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-grow">
        <iframe
          key={iframeKey}
          src={`http://localhost:3000/d-solo/76a7e6c9-d7c5-4f28-ad45-caccb7d2499c/dashbord-dt?orgId=1&timezone=utc&var-sensor_id=dps_light1&var-controller_ref=DPS001&var-Filters=${filters.selectedFilter}&from=${range.from}&to=${range.to}&refresh=auto&theme=light&panelId=18&__feature.dashboardSceneSolo=true`}
          className="w-full h-full border-0"
          title="Vibration Sensor Drill Panel"
        />
      </div>
    </div>
  );
